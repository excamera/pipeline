...
def run_pipe(pipe, sched):
    ...
    #     chunk = result_queue.get(block=False)['chunks']  # TODO: chunks vs. m4schunks
    #     num_m4s += 1
...
class PipelineServer(pipeline_pb2_grpc.PipelineServicer):
    def Submit(self, request, context):
        ...
        try:
            pipe, sched = init_pipe(request)

            runthread = threading.Thread(target=run_pipe, args=(pipe, sched))
            runthread.start()
            ...
            pipe_dir = 'logs/' + pipe.pipe_id
            chunk = result_queue.queue[0]['chunks']  # TODO: should named chunks or m4schunks
            num_m4s = 887
            duration = 887.0
            out_key = chunk['key']
            ...
            logging.info("number of m4s chunks: %d", num_m4s)
            logging.info("total duration: %f", duration)
            segnum = '%08d' % int(chunk['metadata']['lineage'])
            if out_key is not None:
                logging.info('run: '+'aws s3 cp ' + out_key + segnum + '_dash.mpd ' + pipe_dir + '/00000001_dash.mpd')
                os.system('aws s3 cp ' + out_key + segnum + '_dash.mpd ' + pipe_dir + '/00000001_dash.mpd')
                logging.info('mpd downloaded')
                with open(pipe_dir + '/00000001_dash.mpd', 'r') as fin:
                    init_mpd = fin.read()
                ...
                logging.info('mpd uploaded')
                signed_mpd = get_signed_URI(out_key + 'output.xml')
                logging.info('mpd signed, returning (after waiting 20s)')
                time.sleep(5)
                return pipeline_pb2.SubmitReply(success=True, mpd_url=signed_mpd)
