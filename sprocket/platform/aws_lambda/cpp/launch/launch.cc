...
void launchpar_async(int nlaunch, string fn_name, string akid, string secret, string payload, vector<string> lambda_regions) {
    struct LaunchParInvocation invoc;
    vector<string> regions(lambda_regions.begin(), lambda_regions.end());
    invoc = {nlaunch, fn_name, akid, secret, payload,
        regions, std::chrono::steady_clock::now(), std::clock()};
    invocation_queue.enqueue(invoc);
    return;
}

void initialize_launch() {
    std::thread consumer([]()
    {
        while (true) {
            ...
            int req_count = 0;
            while (true) {
                bool found = invocation_queue.try_dequeue(current);
                if (!found) break;
                if (0 == req_count) {
                    invoc = current;
                }
