...
static PyObject *pylaunch_launchpar_async(PyObject *self, PyObject *args);
static PyObject *pylaunch_initialize_launch(PyObject *self, PyObject *args);
...
static PyMethodDef pylaunch_Methods[] = {
    ...
    {"launchpar_async", pylaunch_launchpar_async, METH_VARARGS, "Launch many lambdas in parallel, asynchronously"},
    {"initialize_launch", pylaunch_initialize_launch, METH_VARARGS, "Initialize launch. Make sure call this before calling launchpar_async"},
    {NULL, NULL, 0, NULL }
};
...
static PyObject *pylaunch_launchpar_async(PyObject *self __attribute__((unused)), PyObject *args) {
    int nlaunch;
    char *fn_name, *akid, *secret, *payload;
    PyObject *lambda_regions_obj;
    if (! PyArg_ParseTuple(args, "issssO!", &nlaunch, &fn_name, &akid, &secret, &payload, &PyList_Type, &lambda_regions_obj)) {
    ...
    launchpar_async(nlaunch, string(fn_name), string(akid), string(secret), string(payload), lambda_regions);

    Py_RETURN_NONE;
}

static PyObject *pylaunch_initialize_launch(PyObject *self __attribute__((unused)), PyObject *args __attribute__((unused))) {
